...
\end{equation}
$D$ is the sensor thickness, $\epsilon$ is the dielectric constant, $\mu$ is the mobility and $\rho$ the resistivity.
...
\end{equation}
$I_{0}$ is the leakage current before the irradiation, $\alpha$ is a damage coefficient dependent on particle type and temperature, $\phi$ is the particle fluence, $d$ is the detector thickness, and $A$ is the area. Figure~\ref{fig_leakage_theory} provides a more detailed overview of bulk-damage-induced leakage current in different types of silicon detectors. Moreover, leakage current changes also with the temperature
\begin{equation}
\label{Sil:temp}
    I(T) \propto T^{2}e^{\frac{-E}{2kT}}.
\end{equation}
$T$ is the temperature, $k$ is the Boltzmann constant and $E$ is the band gap energy. 
...
The effective doping of a detector is influenced by radiation-induced defects that exhibit characteristics of either donors or acceptors. The removal of donors due to radiation affects the overall space charge within the detector. When a phosphorus vacancy center is formed, the phosphorus atoms lose their donor properties.

Initially, the detector consists of n-type silicon doped with phosphorus, where phosphorus acts as a donor. However, during irradiation, the material becomes less effectively n-doped. This decrease continues until the number of acceptor states matches the number of remaining donor states, resulting in the material becoming intrinsic. In other words, it loses its net doping effect.

As the irradiation fluence increases, the doping concentration starts to rise again. However, since the material already lacks donors, the newly formed defects tend to act as acceptors, causing the material to become p-doped. This phenomenon is known as space charge inversion.

After depleting the initial doping concentration, a linear increase in negative space charge is observed. This is attributed to the accumulation of acceptor-like defects.

It's important to note that the specific behavior described here may vary depending on the characteristics of the detector material and the nature of the radiation exposure. Figure 2.8 likely provides a visual representation of the changes in depletion voltage and effective doping concentration that occur after irradiation.
...
\begin{itemize}
    ...
    \item momentum resolution - it is mainly influenced by the material budget of the system. The \gls{STS} is designed with the aim of minimizing multiple scattering. It is achieved by placing the electronics, mechanical infrastructure, and cooling outside the active area. For the \gls{STS} the momentum resolution better than $2\%$ is required, what will be achieved by placing the \gls{STS} in a $1\,Tm$ dipole magnet.
    ...
    \item hit rates and readout - the hit rates of charged particles for the inner-most silicon sensors (\mhz{10}/$\mathrm{cm^{2}}$ define the requirements for the readout system (signal shaping time, number of readout channels, etc.)
\end{itemize}
...
\end{equation}
$ENC_{i}$  is the total current noise, $ ENC_{w}$ is the total white noise and $ENC_{1/f}$ is the total flicker noise. 

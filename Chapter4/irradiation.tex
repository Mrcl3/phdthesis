...
A \gls{SEE} in the low voltage module occurred in each part of the irradiation. In both cases, it was possible to recover the functionalities by enabling the channels again. The mean value $\mu$ is given as the ratio of the occurred events divided by the cumulative dose
\begin{equation}
    \mu = \frac{2}{92} = 0.0217\,\mathrm{mGy^{-1}}.
\end{equation}
Therefore, a soft error will occur after $46\,\mathrm{mGy}$. The probability mass function of this distribution is 1 for dose 0, which means that this distribution can't predict the events with the given average rate. In order to increase the statistics a second irradiation campaign took place at SIS18 septum.

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.75\columnwidth]{Chapter4/images/MCBM_crate.png}
    \caption{The probability distribution of one event occurring at different dose levels.}
...
\end{figure}
Figure~\ref{fig:lv_neutrons} shows how the number of failures cumulates with the total neutron dose, where the longer periods without failure indicate a break in SIS18 operation. Similarly, Figure~\ref{fig:lv_neutrons_rate} depicts the dose rate and related \gls{SEE}.
For the 11 low voltage module failures, the rate equals to
   \begin{equation}
  \mu = \frac{11}{49} = 0.2244\mathrm{\,mGy^{-1}}.
\end{equation}
Therefore, a soft error will occur on average after (see Figure~\ref{fig:sis18_lv2})
\begin{equation}
    \mathrm{D}_{\mathrm{LV}}=\mathrm{4.45}\mathrm{\,mGy}.
\end{equation}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.75\columnwidth]{Chapter4/images/SIS18_LV_prob.png}
        \caption{The probability distribution of one event occurring at different dose levels.}
    \label{fig:sis18_lv2}
...
For the high voltage module, the total number of channels that switched off due to the irradiation is 56. Following a similar procedure as for the previous calculation
  \begin{equation}
 \mu = \frac{56}{49} = 1.1429\,\mathrm{mGy^{-1}},
\end{equation}
what translates to:
\begin{equation}
    D_{HV}=0.88\mathrm{\,mGy}.
\end{equation}
If the high voltage module was situated in the same place as the low voltage, a channel switch off could happen after $0.88\mathrm{\,mGy}$. The probability mass function also shows higher values for the HV module. 
